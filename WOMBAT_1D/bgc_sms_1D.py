...
def bgc_sms_2P2Z_varFe(o2, no3, dfe, \
    ...
    p_zoo_excre = 0.01 / d2s    # rate of excretion by zooplankton (/s)
    p_zoo_assim = 0.925         # zooplankton assimilation efficiency
    p_zoo_prefphy = 1.0         # preference of zooplankton for nanophytoplankton
    p_zoo_prefdia = 0.5         # preference of zooplankton for diatoms
    p_zoo_prefdet = 0.25        # preference of zooplankton for detritus
    ###############################
    ####### Meso-Zooplannkton  ####
    ###############################
    p_mes_grz = 0.25           # scaler for rate of zooplankton grazing
    ...
    p_mes_excre = 0.01 / d2s  # rate of excretion by zooplankton (/s)
    p_mes_assim = 0.925         # zooplankton assimilation efficiency
    p_mes_prefphy = 0.25        # preference of mesozooplankton for nanophytoplankton
    p_mes_prefdia = 1.0         # preference of mesozooplankton for diatoms
    p_mes_prefdet = 0.35        # preference of mesozooplankton for detritus
    p_mes_prefzoo = 1.0         # preference of mesozooplankton for microzooplankton
    ######################
    ####### Detritus  ####
    ######################
    p_det_rem = 0.048 / d2s     # remineralisation rate of detritus (/s)
    ...
    mes_mumax = p_mes_grz * Tfunc_hete
    # 1.2 Calculate prey capture rate function (/s)
    zoo_capphy = p_zoo_prefphy*phy_loc
    zoo_capdia = p_zoo_prefdia*dia_loc
    zoo_capdet = p_zoo_prefdet*det_loc
    mes_capphy = p_mes_prefphy*phy_loc
    mes_capdia = p_mes_prefdia*dia_loc
    mes_capdet = p_mes_prefdet*det_loc
    mes_capzoo = p_mes_prefzoo*zoo_loc
    zoo_capt = p_zoo_capcoef * (zoo_capphy + zoo_capdet + zoo_capdia) * (zoo_capphy + zoo_capdet + zoo_capdia)
    mes_capt = p_mes_capcoef * (mes_capphy + mes_capdia + mes_capdet + mes_capzoo) * (mes_capphy + mes_capdia + mes_capdet + mes_capzoo)
    # 1.3 Calculate the realised grazing rate of zooplankton
    zoo_mu = zoo_mumax * zoo_capt / (zoo_mumax + zoo_capt)
    mes_mu = mes_mumax * mes_capt / (mes_mumax + mes_capt)
        # The functional form of grazing creates increasing half-saturation 
        ...
        # always have the same affinity for phytoplankton irrespective of temp
    # 1.4 Collect terms
    zoo_phygrz = zoo_mu * zoo_loc * (zoo_capphy/(zoo_capphy+zoo_capdet+zoo_capdia+eps))         # f21
    zoo_detgrz = zoo_mu * zoo_loc * (zoo_capdet/(zoo_capphy+zoo_capdet+zoo_capdia+eps))         # f21
    zoo_diagrz = zoo_mu * zoo_loc * (zoo_capdia/(zoo_capphy+zoo_capdet+zoo_capdia+eps))         # f21
    zoo_zooexc = zoo_loc * p_zoo_excre * Tfunc_hete * d2s                         # f31
    mes_phygrz = mes_mu * mes_loc * (mes_capphy/(mes_capphy+mes_capdia+mes_capdet+mes_capzoo+eps))         # f21
    mes_diagrz = mes_mu * mes_loc * (mes_capdia/(mes_capphy+mes_capdia+mes_capdet+mes_capzoo+eps))         # f21
    mes_detgrz = mes_mu * mes_loc * (mes_capdet/(mes_capphy+mes_capdia+mes_capdet+mes_capzoo+eps))         # f21
    mes_zoogrz = mes_mu * mes_loc * (mes_capzoo/(mes_capphy+mes_capdia+mes_capdet+mes_capzoo+eps))         # f21
    ...
    zoo_diagrz_extrafe = zoo_diagrz*dia_FeC - zoo_diagrz*p_zoo_optFeC
    ...
    for kk in np.arange(0,len(z_zgrid)-1):
        if zoo_phygrz_extrafe[kk] < 0.0:
            ...
            zoo_phygrz[kk] = zoo_mu[kk] * zoo_loc[kk] * (zoo_capphy[kk]/(zoo_capphy[kk]+zoo_capdet[kk]+zoo_capdia[kk]+eps))         # f21
            zoo_detgrz[kk] = zoo_mu[kk] * zoo_loc[kk] * (zoo_capdet[kk]/(zoo_capphy[kk]+zoo_capdet[kk]+zoo_capdia[kk]+eps))         # f21
            zoo_diagrz[kk] = zoo_mu[kk] * zoo_loc[kk] * (zoo_capdia[kk]/(zoo_capphy[kk]+zoo_capdet[kk]+zoo_capdia[kk]+eps))         # f21
            ...
            zoo_diagrz_extrafe[kk] = zoo_diagrz[kk]*dia_FeC[kk] - zoo_diagrz[kk]*p_zoo_optFeC
        if mes_phygrz_extrafe[kk] < 0.0:
            ...
            mes_phygrz[kk] = mes_mu[kk] * mes_loc[kk] * (mes_capphy[kk]/(mes_capphy[kk]+mes_capdia[kk]+mes_capdet[kk]+mes_capzoo[kk]+eps))         # f21
            mes_diagrz[kk] = mes_mu[kk] * mes_loc[kk] * (mes_capdia[kk]/(mes_capphy[kk]+mes_capdia[kk]+mes_capdet[kk]+mes_capzoo[kk]+eps))         # f21
            mes_detgrz[kk] = mes_mu[kk] * mes_loc[kk] * (mes_capdet[kk]/(mes_capphy[kk]+mes_capdia[kk]+mes_capdet[kk]+mes_capzoo[kk]+eps))         # f21
            mes_zoogrz[kk] = mes_mu[kk] * mes_loc[kk] * (mes_capzoo[kk]/(mes_capphy[kk]+mes_capdia[kk]+mes_capdet[kk]+mes_capzoo[kk]+eps))         # f21
    ...
    ddt_phy = phy_dicupt - (phy_lmort + phy_qmort + zoo_phygrz + mes_phygrz)
    ddt_dia = dia_dicupt - (dia_lmort + dia_qmort + zoo_diagrz + mes_diagrz)
    ddt_pchl = phy_chl_mu - phy_ChlC * (phy_lmort + phy_qmort + zoo_phygrz + mes_phygrz)*12 
    ddt_dchl = dia_chl_mu - dia_ChlC * (dia_lmort + dia_qmort + zoo_diagrz + mes_diagrz)*12 
    ddt_phyfe = phy_dfeupt - phy_FeC * (phy_lmort + phy_qmort + zoo_phygrz + mes_phygrz)
    ddt_diafe = dia_dfeupt - dia_FeC * (dia_lmort + dia_qmort + zoo_diagrz + mes_diagrz)

    ddt_zoo = (zoo_phygrz + zoo_detgrz + zoo_diagrz) * p_zoo_assim - (zoo_zooexc + zoo_qmort) - mes_zoogrz
    ddt_mes = (mes_phygrz + mes_diagrz + mes_detgrz + mes_zoogrz) * p_mes_assim - (mes_mesexc + mes_qmort)
    ddt_zoofe = (zoo_phygrz*phy_FeC + zoo_detgrz*det_FeC + zoo_diagrz*dia_FeC) * p_zoo_assim \
    ...
    ddt_det = (zoo_phygrz + zoo_detgrz + zoo_diagrz) * (1.0-p_zoo_assim) \
              + (mes_phygrz + mes_diagrz + mes_detgrz + mes_zoogrz) * (1.0-p_mes_assim) \
              + phy_qmort + dia_qmort + zoo_qmort + mes_qmort \
              - det_remin - zoo_detgrz - mes_detgrz
    ddt_detfe = (zoo_phygrz*phy_FeC + zoo_detgrz*det_FeC + zoo_diagrz*dia_FeC) * (1.0-p_zoo_assim) \
...
@jit(nopython=True)
def bgc_sms_2P2Z_nh4(o2, nh4, no3, dfe, \
                 phy, dia, zoo, mes, \
                 det, poc, cal, alk, dic, \
                 pchl, dchl, \
    ...
    diafe_loc = diafe[1,:]  # µM Fe
    detfe_loc = detfe[1,:]  # µM Fe
    pocfe_loc = pocfe[1,:]  # µM Fe
    zoofe_loc = zoofe[1,:]  # µM Fe
    mesfe_loc = mesfe[1,:]  # µM Fe
    
    # Make tracer values zero if negative
    o2_loc = np.fmax(o2_loc, 0.0)
    nh4_loc = np.fmax(nh4_loc, 0.0)
    no3_loc = np.fmax(no3_loc, 0.0)
    dfe_loc = np.fmax(dfe_loc, 0.0)
    phy_loc = np.fmax(phy_loc, eps)
    dia_loc = np.fmax(dia_loc, eps)
    ...
    ####### Stoichiometry  ####
    ###########################
    p_CN = 122.0/16.0       # mol/mol
    p_O2C = 172.0/122.0     # mol/mol
    #################################
    ####### Nano-Phytoplannkton  ####
    #################################
    # DIC + NO3 + dFe --> POC + O2
    ...
    p_phy_qmort = 0.1 / d2s     # quadratic mortality of phytoplankton (1 / (µM C * s))
    p_phy_minChlC = 0.004       # minimum chlorophyll : Carbon ratio
    p_phy_maxChlC = 0.033       # maximum chlorophyll : Carbon ratio (reduced by cooler temperatures)
    #####################
    ####### Diatoms  ####
    ...
    p_kdia_no3 = 0.7            # µM
    p_kdia_dfe = 0.1e-3         # µM
    p_dia_lmort = 0.025 / d2s  # linear mortality of phytoplankton (basal respiration) (/s)
    p_dia_qmort = 0.1 / d2s    # quadratic mortality of phytoplankton (1 / (µM C * s))
    p_dia_minChlC = 0.004      # minimum chlorophyll : Carbon ratio
    p_dia_maxChlC = 0.05       # maximum chlorophyll : Carbon ratio (reduced by cooler temperatures)
    ##########################
    ####### Zooplannkton  ####
    ##########################
    p_zoo_grz = 0.5             # scaler for rate of zooplankton grazing
    p_zoo_capcoef = 1.0 / d2s   # prey capture efficiency coefficient (m6 / (µM C)2 * s))
    p_zoo_qmort = 0.05 / d2s    # quadratic mortality of zooplankton (1 / (µM C * s))
    p_zoo_excre = 0.04 / d2s    # rate of excretion by zooplankton (/s)
    p_zoo_assim = 0.70          # zooplankton assimilation efficiency
    p_zoo_prefphy = 1.0         # preference of zooplankton for nanophytoplankton
    p_zoo_prefdia = 0.5         # preference of zooplankton for diatoms
    p_zoo_prefdet = 0.25        # preference of zooplankton for detritus
    p_zoo_prefpoc = 0.10        # preference of zooplankton for large detritus
    ###############################
    ####### Meso-Zooplannkton  ####
    ###############################
    p_mes_grz = 0.25            # scaler for rate of zooplankton grazing
    p_mes_capcoef = 0.5 / d2s   # prey capture efficiency coefficient (m6 / (µM C)2 * s))
    p_mes_qmort = 0.5 / d2s     # quadratic mortality of zooplankton (1 / (µM C * s))
    p_mes_excre = 0.04 / d2s    # rate of excretion by zooplankton (/s)
    p_mes_assim = 0.70          # zooplankton assimilation efficiency
    p_mes_prefphy = 0.25        # preference of mesozooplankton for nanophytoplankton
    p_mes_prefdia = 1.0         # preference of mesozooplankton for diatoms
    p_mes_prefdet = 0.35        # preference of mesozooplankton for detritus
    p_mes_prefpoc = 0.35        # preference of mesozooplankton for large detritus
    p_mes_prefzoo = 1.0         # preference of mesozooplankton for microzooplankton
    ######################
    ####### Detritus  ####
    ######################
    p_det_rem = 0.048 / d2s     # remineralisation rate of detritus (/s)
    p_poc_rem = 0.048 / d2s     # remineralisation rate of detritus (/s)
    ###################
    ####### CaCO3  ####
    ###################
    p_cal_rem = 0.001714 / d2s  # remineralisation rate of CaCO3 (/s)
    ...
    diafe_loc = np.fmax(diafe_loc, eps*p_dia_optFeC)
    detfe_loc = np.fmax(detfe_loc, eps*p_phy_optFeC)
    pocfe_loc = np.fmax(pocfe_loc, eps*p_phy_optFeC)
    zoofe_loc = np.fmax(zoofe_loc, eps*p_zoo_optFeC)
    mesfe_loc = np.fmax(mesfe_loc, eps*p_mes_optFeC)
    ...
    # 1.3 Calculate nutrient limitation terms
    phy_zlnh4 = nh4_loc/(nh4_loc+p_kphy_no3)
    phy_zlno3 = no3_loc/(no3_loc+p_kphy_no3)
    phy_zlnit = (no3_loc + nh4_loc) / (no3_loc + nh4_loc + p_kphy_no3)
    phy_lnh4 = phy_zlnit * 5.0 * phy_zlnh4 / (phy_zlno3 + 5.0 * phy_zlnh4 + eps)
    phy_lno3 = phy_zlnit * phy_zlno3       / (phy_zlno3 + 5.0 * phy_zlnh4 + eps)
    phy_lnit = phy_lnh4 + phy_lno3
    
    dia_zlnh4 = nh4_loc/(nh4_loc+p_kdia_no3)
    dia_zlno3 = no3_loc/(no3_loc+p_kdia_no3)
    dia_zlnit = (no3_loc + nh4_loc) / (no3_loc + nh4_loc + p_kdia_no3)
    dia_lnh4 = dia_zlnit * 5.0 * dia_zlnh4 / (dia_zlno3 + 5.0 * dia_zlnh4 + eps)
    dia_lno3 = dia_zlnit * dia_zlno3       / (dia_zlno3 + 5.0 * dia_zlnh4 + eps)
    dia_lnit = dia_lnh4 + dia_lno3
    ...
    dia_minFeC = p_fe_coef1 * dchl_loc / dia_loc + \
                 ...
                 p_fe_coef3 * dia_lno3
    dia_ldfe = np.fmax(0.0, (dia_FeC - dia_minFeC) / p_dia_optFeC) 
    phy_lnut = np.fmin(phy_lnit, phy_ldfe)
    dia_lnut = np.fmin(dia_lnit, dia_ldfe)
    # 1.4 Calculate the realised growth rate of phytoplankton after light and nutrient limitation
    phy_mu = phy_pro * phy_lnut
    dia_mu = dia_pro * dia_lnut
    # 1.5 Calculate the realised growth rate of chlorophyll
    ...
    # 1.5 Collect terms for phytoplankton
    phy_dicupt = phy_mu * phy_loc                                           # f11
    phy_nh4upt = phy_dicupt / p_CN * (phy_lnh4+eps) / (phy_lnit+eps)        # f11
    phy_no3upt = phy_dicupt / p_CN * (phy_lno3+eps) / (phy_lnit+eps)        # f11
    phy_oxyprd = phy_dicupt * p_O2C
    dia_dicupt = dia_mu * dia_loc                                           # f11
    dia_nh4upt = dia_dicupt / p_CN * (dia_lnh4+eps) / (dia_lnit+eps)        # f11
    dia_no3upt = dia_dicupt / p_CN * (dia_lno3+eps) / (dia_lnit+eps)        # f11
    dia_oxyprd = dia_dicupt * p_O2C
    ...
    zoo_capdia = p_zoo_prefdia*dia_loc
    zoo_capdet = p_zoo_prefdet*det_loc
    zoo_cappoc = p_zoo_prefpoc*poc_loc
    mes_capphy = p_mes_prefphy*phy_loc
    mes_capdia = p_mes_prefdia*dia_loc
    mes_capdet = p_mes_prefdet*det_loc
    mes_cappoc = p_mes_prefpoc*poc_loc
    mes_capzoo = p_mes_prefzoo*zoo_loc
    zoo_capt = p_zoo_capcoef * (zoo_capphy + zoo_capdet + zoo_cappoc + zoo_capdia) * \
               (zoo_capphy + zoo_capdet + zoo_cappoc + zoo_capdia)
    mes_capt = p_mes_capcoef * (mes_capphy + mes_capdia + mes_capdet + mes_cappoc + mes_capzoo) \
               * (mes_capphy + mes_capdia + mes_capdet + mes_cappoc + mes_capzoo)
    # 1.3 Calculate the realised grazing rate of zooplankton
    zoo_mu = zoo_mumax * zoo_capt / (zoo_mumax + zoo_capt)
    mes_mu = mes_mumax * mes_capt / (mes_mumax + mes_capt)
        # The functional form of grazing creates increasing half-saturation 
        ...
        # always have the same affinity for phytoplankton irrespective of temp
    # 1.4 Collect terms
    zoo_phygrz = zoo_mu * zoo_loc * (zoo_capphy/(zoo_capphy+zoo_capdet+zoo_cappoc+zoo_capdia+eps))         # f21
    zoo_detgrz = zoo_mu * zoo_loc * (zoo_capdet/(zoo_capphy+zoo_capdet+zoo_cappoc+zoo_capdia+eps))         # f21
    zoo_pocgrz = zoo_mu * zoo_loc * (zoo_cappoc/(zoo_capphy+zoo_capdet+zoo_cappoc+zoo_capdia+eps))         # f21
    zoo_diagrz = zoo_mu * zoo_loc * (zoo_capdia/(zoo_capphy+zoo_capdet+zoo_cappoc+zoo_capdia+eps))         # f21
    zoo_zooexc = zoo_loc * p_zoo_excre * Tfunc_hete * d2s                         # f31
    mes_phygrz = mes_mu * mes_loc * (mes_capphy/(mes_capphy+mes_capdia+mes_capdet+mes_cappoc+mes_capzoo+eps))         # f21
    mes_diagrz = mes_mu * mes_loc * (mes_capdia/(mes_capphy+mes_capdia+mes_capdet+mes_cappoc+mes_capzoo+eps))         # f21
    mes_detgrz = mes_mu * mes_loc * (mes_capdet/(mes_capphy+mes_capdia+mes_capdet+mes_cappoc+mes_capzoo+eps))         # f21
    mes_pocgrz = mes_mu * mes_loc * (mes_cappoc/(mes_capphy+mes_capdia+mes_capdet+mes_cappoc+mes_capzoo+eps))         # f21
    mes_zoogrz = mes_mu * mes_loc * (mes_capzoo/(mes_capphy+mes_capdia+mes_capdet+mes_cappoc+mes_capzoo+eps))         # f21
    mes_mesexc = mes_loc * p_mes_excre * Tfunc_hete * d2s                        # f31
    # 1.5 Calculate if extra Fe is needed or excreted by zooplankton
    zoo_phygrz_extrafe = zoo_phygrz*phy_FeC - zoo_phygrz*p_zoo_optFeC
    zoo_detgrz_extrafe = zoo_detgrz*det_FeC - zoo_detgrz*p_zoo_optFeC
    zoo_pocgrz_extrafe = zoo_pocgrz*poc_FeC - zoo_pocgrz*p_zoo_optFeC
    zoo_diagrz_extrafe = zoo_diagrz*dia_FeC - zoo_diagrz*p_zoo_optFeC
    mes_phygrz_extrafe = mes_phygrz*phy_FeC - mes_phygrz*p_mes_optFeC
    mes_diagrz_extrafe = mes_diagrz*dia_FeC - mes_diagrz*p_mes_optFeC
    mes_detgrz_extrafe = mes_detgrz*det_FeC - mes_detgrz*p_mes_optFeC
    mes_pocgrz_extrafe = mes_pocgrz*poc_FeC - mes_pocgrz*p_mes_optFeC
    mes_zoogrz_extrafe = mes_zoogrz*zoo_FeC - mes_zoogrz*p_mes_optFeC
    # if negative (Fe needed), then increase the grazing rates of zooplankton by up to 2-fold
    for kk in np.arange(0,len(z_zgrid)-1):
        if zoo_phygrz_extrafe[kk] < 0.0:
            zoo_Fethirst = np.fmin(2.0, (zoo_phygrz[kk]+zoo_detgrz[kk]+zoo_pocgrz[kk]+zoo_diagrz[kk])*p_zoo_optFeC \
                                        / ( zoo_phygrz[kk]*phy_FeC[kk] + zoo_detgrz[kk]*det_FeC[kk] \
                                          + zoo_pocgrz[kk]*poc_FeC[kk] + zoo_diagrz[kk]*dia_FeC[kk]) )
            zoo_mumax[kk] = p_zoo_grz * Tfunc_hete[kk] * zoo_Fethirst
            zoo_mu[kk] = zoo_mumax[kk] * zoo_capt[kk] / (zoo_mumax[kk] + zoo_capt[kk])
            zoo_phygrz[kk] = zoo_mu[kk] * zoo_loc[kk] * (zoo_capphy[kk]/(zoo_capphy[kk]+zoo_capdet[kk]+zoo_cappoc[kk]+zoo_capdia[kk]+eps))         # f21
            zoo_detgrz[kk] = zoo_mu[kk] * zoo_loc[kk] * (zoo_capdet[kk]/(zoo_capphy[kk]+zoo_capdet[kk]+zoo_cappoc[kk]+zoo_capdia[kk]+eps))         # f21
            zoo_pocgrz[kk] = zoo_mu[kk] * zoo_loc[kk] * (zoo_cappoc[kk]/(zoo_capphy[kk]+zoo_capdet[kk]+zoo_cappoc[kk]+zoo_capdia[kk]+eps))         # f21
            zoo_diagrz[kk] = zoo_mu[kk] * zoo_loc[kk] * (zoo_capdia[kk]/(zoo_capphy[kk]+zoo_capdet[kk]+zoo_cappoc[kk]+zoo_capdia[kk]+eps))         # f21
            zoo_phygrz_extrafe[kk] = zoo_phygrz[kk]*phy_FeC[kk] - zoo_phygrz[kk]*p_zoo_optFeC
            zoo_detgrz_extrafe[kk] = zoo_detgrz[kk]*det_FeC[kk] - zoo_detgrz[kk]*p_zoo_optFeC
            zoo_pocgrz_extrafe[kk] = zoo_pocgrz[kk]*poc_FeC[kk] - zoo_pocgrz[kk]*p_zoo_optFeC
            zoo_diagrz_extrafe[kk] = zoo_diagrz[kk]*dia_FeC[kk] - zoo_diagrz[kk]*p_zoo_optFeC
        if mes_phygrz_extrafe[kk] < 0.0:
            mes_Fethirst = np.fmin(2.0, 
                                   (mes_phygrz[kk]+mes_diagrz[kk]+mes_detgrz[kk]+mes_pocgrz[kk]+mes_zoogrz[kk])*p_zoo_optFeC \
                                   / ( mes_phygrz[kk]*phy_FeC[kk] + mes_diagrz[kk]*dia_FeC[kk] + \
                                       mes_detgrz[kk]*det_FeC[kk] + mes_pocgrz[kk]*poc_FeC[kk] + mes_zoogrz[kk]*p_zoo_optFeC) )
            mes_mumax[kk] = p_mes_grz * Tfunc_hete[kk] * mes_Fethirst
            mes_mu[kk] = mes_mumax[kk] * mes_capt[kk] / (mes_mumax[kk] + mes_capt[kk])
            mes_phygrz[kk] = mes_mu[kk] * mes_loc[kk] * (mes_capphy[kk]/(mes_capphy[kk]+mes_capdia[kk]+mes_capdet[kk]+mes_cappoc[kk]+mes_capzoo[kk]+eps))         # f21
            mes_diagrz[kk] = mes_mu[kk] * mes_loc[kk] * (mes_capdia[kk]/(mes_capphy[kk]+mes_capdia[kk]+mes_capdet[kk]+mes_cappoc[kk]+mes_capzoo[kk]+eps))         # f21
            mes_detgrz[kk] = mes_mu[kk] * mes_loc[kk] * (mes_capdet[kk]/(mes_capphy[kk]+mes_capdia[kk]+mes_capdet[kk]+mes_cappoc[kk]+mes_capzoo[kk]+eps))         # f21
            mes_pocgrz[kk] = mes_mu[kk] * mes_loc[kk] * (mes_cappoc[kk]/(mes_capphy[kk]+mes_capdia[kk]+mes_capdet[kk]+mes_cappoc[kk]+mes_capzoo[kk]+eps))         # f21
            mes_zoogrz[kk] = mes_mu[kk] * mes_loc[kk] * (mes_capzoo[kk]/(mes_capphy[kk]+mes_capdia[kk]+mes_capdet[kk]+mes_cappoc[kk]+mes_capzoo[kk]+eps))         # f21
            mes_phygrz_extrafe[kk] = mes_phygrz[kk]*phy_FeC[kk] - mes_phygrz[kk]*p_mes_optFeC
            mes_diagrz_extrafe[kk] = mes_diagrz[kk]*dia_FeC[kk] - mes_diagrz[kk]*p_mes_optFeC
            mes_detgrz_extrafe[kk] = mes_detgrz[kk]*det_FeC[kk] - mes_detgrz[kk]*p_mes_optFeC
            mes_pocgrz_extrafe[kk] = mes_pocgrz[kk]*poc_FeC[kk] - mes_pocgrz[kk]*p_mes_optFeC
            mes_zoogrz_extrafe[kk] = mes_zoogrz[kk]*zoo_FeC[kk] - mes_zoogrz[kk]*p_mes_optFeC
    ...
    # (3) Mortality terms
    #----------------------------------------------------------------------
    phy_slmor = np.fmin(1.0, np.fmax(0.0, phy_lnut/0.3))
    dia_slmor = np.fmin(1.0, np.fmax(0.0, dia_lnut/0.3))
    #phy_slmor = 1.0
    #dia_slmor = 1.0
    phy_lmort = p_phy_lmort * phy_loc * Tfunc_hete * d2s * phy_slmor           # f22
    phy_qmort = p_phy_qmort * phy_loc * phy_loc # Not temperature-dependent?   # f23
    dia_lmort = p_dia_lmort * dia_loc * Tfunc_hete * d2s * dia_slmor           # f22
    dia_qmort = p_dia_qmort * dia_loc * dia_loc # Not temperature-dependent?   # f23
    zoo_qmort = p_zoo_qmort * zoo_loc * zoo_loc # Not temperature-dependent?   # f32
    mes_qmort = p_mes_qmort * mes_loc * mes_loc # Not temperature-dependent?   # f32
    ...
    #----------------------------------------------------------------------
    det_remin = p_det_rem * det_loc * Tfunc_hete * d2s                    # f41
    poc_remin = p_poc_rem * poc_loc * Tfunc_hete * d2s                    # f41
    cal_remin = p_cal_rem * cal_loc                                       # f51
    nitrif = nh4_loc * 0.05 / d2s

    #----------------------------------------------------------------------
    # (5)  Tracer equations (tendencies) Sources - Sinks
    #----------------------------------------------------------------------
    ddt_o2 = phy_oxyprd + dia_oxyprd \
             - (phy_lmort + dia_lmort + zoo_zooexc + mes_mesexc + det_remin + poc_remin) * p_O2C
             
    ddt_nh4 = (phy_lmort + dia_lmort + zoo_zooexc + mes_mesexc + det_remin + poc_remin) / p_CN \
              - phy_nh4upt - dia_nh4upt - nitrif
    
    ddt_no3 = nitrif - phy_no3upt - dia_no3upt
    ...
    ddt_zoo = (zoo_phygrz + zoo_detgrz + zoo_pocgrz + zoo_diagrz) * p_zoo_assim - (zoo_zooexc + zoo_qmort) - mes_zoogrz
    ddt_mes = (mes_phygrz + mes_diagrz + mes_detgrz + mes_pocgrz + mes_zoogrz) * p_mes_assim - (mes_mesexc + mes_qmort)
    ddt_zoofe = (zoo_phygrz*phy_FeC + zoo_detgrz*det_FeC + zoo_pocgrz*poc_FeC + zoo_diagrz*dia_FeC) * p_zoo_assim \
                - (zoo_zooexc + zoo_qmort + mes_zoogrz)*zoo_FeC
    ddt_mesfe = (mes_phygrz*phy_FeC + mes_diagrz*dia_FeC + mes_detgrz*det_FeC + mes_pocgrz*poc_FeC + mes_zoogrz*zoo_FeC) * p_mes_assim \
                - (mes_mesexc + mes_qmort)*mes_FeC
                
    ddt_det = (zoo_phygrz + zoo_detgrz + zoo_pocgrz + zoo_diagrz) * (1.0-p_zoo_assim) * 0.7 \
              + (mes_phygrz + mes_diagrz + mes_detgrz + mes_pocgrz + mes_zoogrz) * (1.0-p_mes_assim) * 0.7 \
              + (phy_qmort + dia_qmort + zoo_qmort + mes_qmort) * 0.7 \
              - det_remin - zoo_detgrz - mes_detgrz
    ddt_detfe = (zoo_phygrz*phy_FeC + zoo_detgrz*det_FeC + zoo_pocgrz*poc_FeC + zoo_diagrz*dia_FeC) * (1.0-p_zoo_assim) * 0.7 \
                + (mes_phygrz*phy_FeC + mes_diagrz*dia_FeC + mes_detgrz*det_FeC + mes_pocgrz*poc_FeC + mes_zoogrz*zoo_FeC) * (1.0-p_mes_assim) * 0.7 \
                + (phy_qmort*phy_FeC + dia_qmort*dia_FeC + zoo_qmort*zoo_FeC + mes_qmort*mes_FeC) * 0.7 \
                - (det_remin + zoo_detgrz + mes_detgrz) * det_FeC #\
                #+ p_dfe_scav * np.fmax(0.0, dfe_loc - p_dfe_deep)

    ddt_poc = (zoo_phygrz + zoo_detgrz + zoo_pocgrz + zoo_diagrz) * (1.0-p_zoo_assim) * 0.3 \
              + (mes_phygrz + mes_diagrz + mes_detgrz + mes_pocgrz + mes_zoogrz) * (1.0-p_mes_assim) * 0.3 \
              + (phy_qmort + dia_qmort + zoo_qmort + mes_qmort) * 0.3 \
              - poc_remin - zoo_pocgrz - mes_pocgrz
    ddt_pocfe = (zoo_phygrz*phy_FeC + zoo_detgrz*det_FeC + zoo_pocgrz*poc_FeC + zoo_diagrz*dia_FeC) * (1.0-p_zoo_assim) * 0.3 \
                + (mes_phygrz*phy_FeC + mes_diagrz*dia_FeC + mes_detgrz*det_FeC + mes_pocgrz*poc_FeC + mes_zoogrz*zoo_FeC) * (1.0-p_mes_assim) * 0.3 \
                + (phy_qmort*phy_FeC + dia_qmort*dia_FeC + zoo_qmort*zoo_FeC + mes_qmort*mes_FeC) * 0.3 \
                - (poc_remin + zoo_pocgrz + mes_pocgrz) * poc_FeC #\
                #+ p_dfe_scav * np.fmax(0.0, dfe_loc - p_dfe_deep)
    ...
    ddt_alk = ddt_nh4 - ddt_no3 - ddt_cal * 2.0
    
    ddt_dic = ((ddt_no3 + ddt_nh4) * p_CN) - ddt_cal
    
    #----------------------------------------------------------------------
    # (6)  Diagnostics for evaluation
    #----------------------------------------------------------------------
    pgi_zoo = (zoo_mu * zoo_loc) / (phy_loc + det_loc + poc_loc + dia_loc)
    pgi_mes = (mes_mu * mes_loc) / (phy_loc + dia_loc + det_loc + poc_loc + zoo_loc)
    

    return [ddt_o2, ddt_nh4 , ddt_no3, ddt_dfe, ddt_phy, ddt_dia, ddt_zoo, ddt_mes, ddt_det, ddt_poc, \
            ddt_cal, ddt_alk, ddt_dic, ddt_pchl, ddt_dchl, \
            ddt_phyfe, ddt_diafe, ddt_detfe, ddt_pocfe, ddt_zoofe, ddt_mesfe, \
            pgi_zoo, pgi_mes, \
            phy_mu, dia_mu, zoo_mu, mes_mu, phy_lmort, dia_lmort, phy_qmort, dia_qmort, \
            phy_ChlC, dia_ChlC, phy_FeC*1e6, dia_FeC*1e6, det_FeC*1e6, poc_FeC*1e6, zoo_FeC*1e6, mes_FeC*1e6]
